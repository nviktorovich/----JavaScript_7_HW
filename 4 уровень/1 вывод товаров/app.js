...
let elementForInsert = document.querySelector(".products")
...
function clickHandler(event) {
    ...
    elementForInsert.textContent = ""
    for (let i = 0; i < buttons.length; i++) {
        if (event.target === buttons[i]) {
            showCategory(buttons[i].attributes[dataType].value)
        }
    }
}
...
function showCategory(category) {
    ...
    let productLine = products[category]
    console.log(productLine)
    for (let i = 0; i < productLine.length; i++) {
        getProductMarkup(productLine[i])
    }
}
...
function getProductMarkup(product) {
    console.log(product.id, product.imageUrl, product.name, product.price)
    elementForInsert.insertAdjacentHTML("afterbegin", `<div>${product.name}</div>
